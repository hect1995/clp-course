function [knn_Pe_train, CM_knn_train, knn_Pe_test, CM_knn_test] = prac4_MA_function( K_neig )
%Lectura de la base de datos de los MICROARRAY
% clear
close all
% Lectura de etiquetas
% load ref.txt
Labels_train = (load('cancer/14cancer.ytrain'))';
Labels_test = (load('cancer/14cancer.ytest'))';
% Lectura de datos
% X = textread('nci.txt','','delimiter',' ');
X = textread('cancer/14cancer.xtrain','','delimiter',' ');
X_train=X';

X = textread('cancer/14cancer.xtest','','delimiter',' ');
X_test=X';
% Labels=ref;
clear X
%OPCIONES
...
i_dib=0;                                %0 NO /1 SI: DIBUJOS DE VECTORES
